...
export default function InteractiveMap({ language }: InteractiveMapProps) {
  ...
  const centers = [
    { 
      ...
      city: 'Axel',
      cityAr: 'Axel',
      address: 'Meekrapweg 2 – 4571 RX',
      addressAr: 'Meekrapweg 2 – 4571 RX',
      services: ['LCL', 'FCL'],
  ...
  return (
    <div className="w-full">
      <h3 className="text-3xl md:text-4xl font-bold mb-4 text-center text-primary-dark">{t.title}</h3>
      ...
      <div className="w-full max-w-6xl mx-auto bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl shadow-2xl p-8 relative overflow-hidden">
        ...
        <div className="relative w-full h-[500px] bg-white rounded-xl shadow-inner flex items-center justify-center overflow-hidden">
          ...
          >
            ...
            <line x1="400" y1="250" x2="580" y2="350" stroke="#FFD200" strokeWidth="3" strokeDasharray="10,5" opacity="0.4"/>
          </svg>

          {/* Grid lines for visual effect */}
          <div className="absolute inset-0 opacity-10">
            {[...Array(10)].map((_, i) => (
              <div key={`h-${i}`} className="absolute w-full border-t border-gray-400" style={{ top: `${i * 10}%` }}></div>
            ))}
            {[...Array(12)].map((_, i) => (
              <div key={`v-${i}`} className="absolute h-full border-l border-gray-400" style={{ left: `${i * 8.33}%` }}></div>
            ))}
          </div>
              ...
              <div className="relative">
                ...
                {hoveredCenter === center.id && (
                  <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 bg-primary-dark text-white px-5 py-3 rounded-lg shadow-xl z-20 animate-fade-in min-w-[220px]">
                    <div className="text-2xl mb-1">{center.flag}</div>
                    <div className="font-bold text-base">{language === 'ar' ? center.countryAr : center.country}</div>
                    <div className="text-sm text-primary-yellow">{language === 'ar' ? center.cityAr : center.city}</div>
                    {/* Arrow pointer */}
        ...
        <div className="mt-6 flex items-center justify-center gap-3 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-700 font-medium">
              {language === 'ar' ? 'مركز تجميع وشحن' : 'Collection & Shipping Center'}
            </span>
          </div>
        </div>
      </div>
        ...
        {centers.map((center) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-gray-200">
              <div className="text-3xl">{center.flag}</div>
              <div>
            ...
            <div className="mb-3">
              ...
              <div className="flex items-center gap-2">
                <svg className="w-4 h-4 text-primary-yellow flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">

...
export default function Step4ParcelDetails({
    ...
    const updatedParcels = parcels.map(parcel => {
      if (parcel.id === id) {
        ...
        if (field === 'length' || field === 'width' || field === 'height') {
          updatedParcel.cbm = calculateCBM(
            updatedParcel.length || 0,
            updatedParcel.width || 0,
            updatedParcel.height || 0
          );
        }
  ...
  const handlePhotoUpload = (id: string, files: FileList | null, isDevicePhoto: boolean = false) => {
    ...
    <div className="space-y-6">
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-primary-dark">
                {language === 'ar' ? `طرد #${index + 1}` : `Parcel #${index + 1}`}
              </h3>
              <motion.button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateParcel(parcel.id, 'productCategory', e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateParcel(parcel.id, 'quantity', parseInt(e.target.value) || 1)}
                  className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow"
                />
              </div>
              ...
              <div className="md:col-span-2">
                ...
                {parcel.photos && parcel.photos.length > 0 && (
                  <p className="mt-2 text-sm text-gray-600">
                    {parcel.photos.length} {language === 'ar' ? 'صورة محملة' : 'photos uploaded'}
                  </p>
                )}
              </div>
              ...
              {hasElectronics && (
                ...
                >
                  <h4 className="font-bold text-purple-900">{t.electronicsInfo}</h4>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateParcel(parcel.id, 'deviceType', e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow bg-white"
                    >
                      <option value="">{language === 'ar' ? 'اختر...' : 'Select...'}</option>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateParcel(parcel.id, 'deviceModel', e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateParcel(parcel.id, 'declaredValue', parseFloat(e.target.value) || 0)}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow"
                    />
                  </div>
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) => updateParcel(parcel.id, 'hasInvoice', e.target.checked)}
                      className="w-5 h-5 text-primary-yellow rounded focus:ring-primary-yellow"
                    />
                    <label className="ml-2 text-sm text-gray-700">{t.hasInvoice}</label>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => handlePhotoUpload(parcel.id, e.target.files, true)}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow"
                    />
                  </div>
                </motion.div>
              ...
              {hasLargeItems && (
                ...
                >
                  <h4 className="font-bold text-orange-900">{t.largeItemInfo}</h4>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateParcel(parcel.id, 'itemType', e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow bg-white"
                    >
                      <option value="">{language === 'ar' ? 'اختر...' : 'Select...'}</option>
                      {itemTypes.map(type => (
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e) => updateParcel(parcel.id, 'unknownDimensions', e.target.checked)}
                      className="w-5 h-5 text-primary-yellow rounded focus:ring-primary-yellow"
                    />
                    <label className="ml-2 text-sm text-gray-700">{t.unknownDimensions}</label>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      value={parcel.notes || ''}
                      onChange={(e) => updateParcel(parcel.id, 'notes', e.target.value)}
                      rows={3}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-300 focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow"
                    />
                  </div>
      ...
      {parcels.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          {language === 'ar' ? 'لا توجد طرود. اضغط على "إضافة طرد جديد" للبدء.' : 'No parcels. Click "Add New Parcel" to start.'}
        </div>
      )}
    </div>
  );

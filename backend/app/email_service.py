...
def send_fcl_invoice_email_to_user(quote, pdf_bytes):
    """
    Send FCL invoice PDF to user via email.
    ...
    # Check if email is configured
    if not settings.EMAIL_HOST_USER or not settings.EMAIL_HOST_PASSWORD:
        logger.warning("Email not configured. Skipping FCL invoice email to user.")
        return False
    ...
    try:
        ...
        if not user_email:
            logger.warning(f"No email found for FCL quote {quote.id}")
            return False
        
        # Email subject
        subject_ar = f"فاتورتك جاهزة - عرض السعر {quote.quote_number or quote.id}"
        subject_en = f"Your Invoice is Ready - Quote {quote.quote_number or quote.id}"
        subject = subject_ar  # Default to Arabic
        
        # Email body
        body_ar = f"""
        مرحباً {quote.full_name},
        
        تم توليد فاتورتك بنجاح لعرض السعر رقم {quote.quote_number or quote.id}.
        
        يمكنك تحميل الفاتورة من المرفقات أو من لوحة التحكم الخاصة بك.
        
        تفاصيل عرض السعر:
        - رقم عرض السعر: {quote.quote_number or quote.id}
        - الحالة: {quote.get_status_display()}
        - المبلغ الإجمالي: €{quote.total_price or 0}
        - المبلغ المدفوع: €{quote.amount_paid or 0}
        ...
        Your invoice has been successfully generated for quote {quote.quote_number or quote.id}.
        
        You can download the invoice from the attachments or from your dashboard.
        
        Quote Details:
        - Quote Number: {quote.quote_number or quote.id}
        - Status: {quote.get_status_display()}
        - Total Amount: €{quote.total_price or 0}
        - Amount Paid: €{quote.amount_paid or 0}
        ...
        body = body_ar  # Default to Arabic
        ...
        # Attach PDF
        email.attach(
            f"FCL-Invoice-{quote.quote_number or quote.id}.pdf",
            pdf_bytes,
            'application/pdf'
        )
        ...
        logger.info(f"✅ FCL invoice email sent to user {user_email} for quote {quote.id}")
        return True
        
    except Exception as e:
        logger.error(
            f"Error sending FCL invoice email to user: {str(e)}",
            exc_info=True
        )
        return False


def send_fcl_invoice_email_to_admin(quote, pdf_bytes):
    """
    Send FCL invoice PDF to admin via email.
    ...
    # Check if email is configured
    if not settings.EMAIL_HOST_USER or not settings.EMAIL_HOST_PASSWORD:
        logger.warning("Email not configured. Skipping FCL invoice email to admin.")
        return False
    ...
    try:
        ...
        user_name = (
            quote.user.get_full_name() or quote.user.username
            if quote.user
            else quote.full_name
        )
        user_email = quote.user.email if quote.user else quote.email
        
        # Email subject
        subject = f"FCL Invoice Generated - Quote {quote.quote_number or quote.id}"
        
        # Email body
        body = f"""
        Invoice has been generated for FCL quote {quote.quote_number or quote.id}.
        
        Quote Details:
        - Quote Number: {quote.quote_number or quote.id}
        - User: {user_name} ({user_email})
        - Status: {quote.get_status_display()}
        - Payment Status: {quote.payment_status or 'N/A'}
        - Total Amount: €{quote.total_price or 0}
        - Amount Paid: €{quote.amount_paid or 0}
        - Payment Method: {quote.get_payment_method_display() if quote.payment_method else 'N/A'}
        ...
        logger.info(f"✅ FCL invoice email sent to admin {admin_email} for quote {quote.id}")
        return True
        
    except Exception as e:
        logger.error(
            f"Error sending FCL invoice email to admin: {str(e)}",
            exc_info=True
        )
        return False